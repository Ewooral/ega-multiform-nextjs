...
const SixDigitPinStep: React.FC<StepProps> = ({
  formMethods,
}) => {
  const {
    register,
  ...
  const {setCustomerSixDigitPIN, serverResponseGenerateOTP } = useRegistrationStore();
  const watchCustomerSixDigitPin = watch("customerSixDigitPIN");


  useEffect(() => {
    setCustomerSixDigitPIN(
       watchCustomerSixDigitPin,
    );
    trigger("firstName");
